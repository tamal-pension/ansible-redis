...
MAIN_SH_ARGS = <<MARKER
-e "playbook_name=ansible-redis discord_message_owner_name=#{Etc.getpwuid(Process.uid).name} environment_id=pension-stg.local" --tags "installation,configuration,debug"
MARKER
Vagrant.configure("2") do |config|
  ...
  config.vm.provider :aws do |aws, override|
    ...
    aws.security_groups = ["sg-077f8d7d58d420467", "sg-0f287cc27125c928b"]
    # public-ssh, redis
    ...
    aws.tags = {
        Name: "redis-test-#{Etc.getpwuid(Process.uid).name}",
        private_dns: "redis-test-#{Etc.getpwuid(Process.uid).name}",
        _replicaof: "redis01.pension-stg.local",
